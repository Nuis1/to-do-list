...
<div class="bg-white shadow-sm hover:shadow-md duration-200 ease-in-out border-l-4 md:border-l-6 <?= $tugas['meta']['border'] ?> rounded-lg md:rounded-2xl p-4 md:p-5">
    <div class="flex flex-col sm:flex-row sm:justify-between gap-4">
        <div class="flex gap-3 md:gap-4 flex-1">
            ...
            <?php else: ?>
                <div class="w-6 h-6 md:w-8 md:h-8 border-2 border-indigo-600 rounded-full"></div>
            <?php endif; ?>
            <div class="flex-1 min-w-0">
                <p class="font-medium text-base md:text-lg mb-1 <?= $tugas['status']==='selesai'?'line-through':'' ?>"><?= htmlspecialchars($tugas['judul']) ?></p>
                <p class="text-sm md:text-base text-gray-500 line-clamp-2 <?= $tugas['status']==='selesai'?'line-through':''?>">Lorem ipsum dolor sit amet consectetur adipiscing elit quisque faucibus.</p>
                <div class="flex flex-col sm:flex-row gap-3 md:gap-6 mt-3 md:mt-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-sm md:text-base <?= strpos($tugas['meta']['label'],'Terlambat')!==false?'text-red-600':'text-blue-600' ?>"><?= $tugas['meta']['info'] ?></p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
